...
interface UploadFileProps {
  hashes: any[];
  isHashExist: boolean;
  setIsHashExist: (value: boolean) => void;
  setExistedHash: (hash: string) => void;
  isFileHashed: boolean;
  setIsFileHashed: (value: boolean) => void;
  existedHash: string | null;
}
...
const UploadFile = forwardRef(
  (
    { hashes, isHashExist, setIsHashExist, setExistedHash, existedHash, isFileHashed, setIsFileHashed }: UploadFileProps,
    ref: React.Ref<{ resetAllStates: () => void }>
  ) => {
    ...
    // show toast error and reset the states if the hash already exist
    // useEffect(() => {
    //   if (!isFileHashing && !isRequestedToOpenWallet && isFileHashed && isHashExist) {
    //     setIsHashExist(false);
    //   }
    // }, [isFileHashed, isFileHashing, isHashExist, isRequestedToOpenWallet, setIsHashExist]);
    ...
    // check if the current hash is not null and th existed hash is not null and current hash is different from existed hash, then set isHashExist to false
    useEffect(() => {
      if (hash && existedHash && hash !== existedHash) {
        setIsHashExist(false);
      }
    }, [hash, setExistedHash]);
    ...
    // list all states that are responsible to show the drag and drop element
    console.log("isFileHashing : ", isFileHashing);
    console.log("isRequestedToOpenWallet : ", isRequestedToOpenWallet);
    console.log("isFileHashed : ", isFileHashed);
    // print isHashExist
    ...
    // display this result (!isFileHashing && !isRequestedToOpenWallet && !isFileHashed ), by saying the comp will be shown or not
    if (!isFileHashing && !isRequestedToOpenWallet && !isFileHashed) {
      console.log("The component will be shown");
    } else {
      console.log("The component will not be shown");
    }
    ...
    return (
      <div>
        ...
        >
          ...
          {!isFileHashing && !isRequestedToOpenWallet && isFileHashed && !isHashExist && (
